...
void FlowField::setRotate(float mouseX, float mouseY) {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ofVec2f pos = ofVec2f( x*resolution, y*resolution );
            ofVec2f preDistance = ofVec2f(mouseX, mouseY) - pos;
            float dist = preDistance.length();
            float speed = ofMap(dist, 0.0, 100.0, 0.5f, 0.03f);
            float str = ofMap(dist, 0.0, 100.0, 1.0f, 10.0f);
            flowList[y][x].set( ofVec2f( cos( ofGetElapsedTimef()*speed)*str, sin( ofGetElapsedTimef()*speed)*str ) );
        }
    }
}

void FlowField::setPattern() {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ofVec2f pos = ofVec2f( x*resolution, y*resolution );
            flowList[y][x].set( ofVec2f( cos(pos.x + ofGetElapsedTimef()*0.5)*10, sin(pos.y + ofGetElapsedTimef()*0.2)*10 ) );
        }
    }
}
